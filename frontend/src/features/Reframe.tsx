...
import { Upload, Sparkles, Image as ImageIcon, Loader2, X, Download, Plus } from "lucide-react";
...
// Fixed Credits for Reframe (Adjust as per your pricing)
const calculateCredits = (res: string) => {
  // Assuming similar cost logic, change if needed
  return 1; // 1 credit for any resolution
};

export function Reframe() {
  const navigate = useNavigate();
  // Input States
  const [image, setImage] = useState<string>(""); // ✅ Only ONE image allowed
  ...
  const estimatedCredits = calculateCredits(resolution);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setImage(base64); // ✅ Replace previous image (Only one at a time)
    }
  };

  // ✅ Remove Image Handler (Small Cross Button)
  const removeImage = () => {
    setImage("");
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Note: Prompt is NOT sent to backend as per your requirement (disabled)
      const data = await runGeneration("reframe", inputs);
      ...
      setError(err instanceof Error ? err.message : "Failed to reframe image");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportDownload = () => {
    ...
    img.onload = () => {
      ...
      link.href = dataUrl;
      link.download = `fashn-reframe.${exportFormat}`;
      link.click();
    };
  };
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-[#f2eee6] to-[#faf9f7] dark:from-[#121212] dark:to-[#1a1a1a] transition-colors duration-300 flex flex-col">
      ...
      <div className="pt-8 pb-4 text-center border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-black/20 backdrop-blur-sm">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Change aspect ratio and resolution of your image</p>
      </div>
        ...
        <div className="lg:w-1/2 flex flex-col gap-5 overflow-y-auto pb-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800">
            <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 flex items-center gap-2 mb-3">
              <Upload className="w-4 h-4 text-[#b5652a]" /> Upload Image
            </label>
          ...
          {/* Disabled Prompt Section */}
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800 opacity-60 pointer-events-none">
            <div>
              <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 mb-1 block flex items-center gap-2">
                Prompt (Disabled)
                <span className="text-[10px] bg-zinc-200 dark:bg-zinc-700 px-1.5 py-0.5 rounded text-zinc-500">Not Available</span>
              </label>
              <input 
                type="text"
                value={prompt}
                ...
                className="w-full border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 rounded-md p-2 text-sm cursor-not-allowed text-zinc-400"
              />
            </div>
          </div>
        ...
        <div className="lg:w-1/2 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex flex-col min-h-[500px] relative">
          ...
          {loading && (
            <div className="absolute inset-0 bg-white/80 dark:bg-zinc-900/90 flex flex-col items-center justify-center z-10 rounded-xl backdrop-blur-sm">
              ...
              <p className="font-medium text-zinc-800 dark:text-white">Reframing your image...</p>
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            {results.length > 0 ? (
               results.map((url, idx) => (
                <div key={idx} className="relative bg-zinc-50 dark:bg-zinc-800 rounded-lg overflow-hidden h-full flex items-center justify-center border border-zinc-100 dark:border-zinc-700 group">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          >
            {loading ? (
              ...
              <>Run Reframe <ImageIcon className="w-4 h-4" /></>
            )}
          </button>
        </div>
      </div>
