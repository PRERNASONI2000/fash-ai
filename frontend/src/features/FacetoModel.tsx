//features/FacetoModel.tsx
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { runGeneration } from "../lib/fashnService";
import { Upload, User, Loader2, X, Download, Plus, Sparkles } from "lucide-react";
...
// ✅ Exact Credit Calculation from Docs
...
export function FacetoModel() {
  ...
  // Setting States
  const [aspectRatio, setAspectRatio] = useState("2:3"); // Default 2:3 as per docs
  ...
  const handleGenerate = async () => {
    ...
    try {
      const inputs: Record<string, any> = {
        face_image: faceImage, // ✅ 'face_image' field name
        prompt: prompt || undefined,
        aspect_ratio: aspectRatio,
        resolution: resolution,
        generation_mode: generationMode,
        num_images: numImages,
        output_format: "jpeg" // Default is jpeg in docs
      };

      const data = await runGeneration("face-to-model", inputs);
      
      // ✅ HANDLING UNIQUE RESPONSE: { output: { images: [...] } }
      let extractedUrls: string[] = [];
      if (Array.isArray(data.output)) {
        extractedUrls = data.output;
      } else if (data.output && typeof data.output === 'object' && Array.isArray(data.output.images)) {
        extractedUrls = data.output.images;
  ...
  const handleExportDownload = () => {
    ...
    img.onload = () => {
      ...
      link.href = dataUrl;
      link.download = `fashn-face-to-model.${exportFormat}`;
      link.click();
    };
  };
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-[#f2eee6] to-[#faf9f7] dark:from-[#121212] dark:to-[#1a1a1a] transition-colors duration-300 flex flex-col">
      ...
      <div className="pt-8 pb-4 text-center border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-black/20 backdrop-blur-sm">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Transform face images into try-on ready avatars</p>
      </div>
        ...
        <div className="lg:w-1/2 flex flex-col gap-5 overflow-y-auto pb-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800">
            <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 flex items-center gap-2 mb-3">
              <Upload className="w-4 h-4 text-[#b5652a]" /> Face Image (Headshot)
            </label>
            
            {faceImage ? (
              <div className="relative w-full h-64 rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-700 group">
                <img src={faceImage} alt="Preview" className="w-full h-full object-contain bg-zinc-100 dark:bg-zinc-800" />
                <button 
                  onClick={removeImage}
              ...
              <label className="w-full h-64 flex flex-col items-center justify-center border-2 border-dashed border-zinc-300 dark:border-zinc-600 rounded-lg cursor-pointer hover:border-[#b5652a] transition-colors bg-zinc-50 dark:bg-zinc-800/50">
                ...
                <span className="text-sm text-zinc-400 font-medium">Click to Upload Face</span>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800">
            <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 mb-1 block">
              Body Shape Guidance (Optional)
            </label>
            <textarea 
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. athletic build, curvy figure, slender frame"
              rows={3}
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800 space-y-4">
            ...
            {/* ✅ Aspect Ratio: ONLY VERTICAL ALLOWED */}
            <div>
              <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-2 block">Aspect Ratio (Vertical Only)</label>
              <SettingToggle 
                options={[
                  { label: "1:1", value: "1:1" },
                  { label: "2:3", value: "2:3" },
        ...
        <div className="lg:w-1/2 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex flex-col min-h-[500px] relative">
          ...
          {loading && (
            <div className="absolute inset-0 bg-white/80 dark:bg-zinc-900/90 flex flex-col items-center justify-center z-10 rounded-xl backdrop-blur-sm">
              ...
              <p className="font-medium text-zinc-800 dark:text-white">Generating avatar...</p>
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            {results.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-zinc-400">
                ...
                <p className="text-sm">Avatars will appear here</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          >
            {loading ? (
              ...
              <>Run Generation <Sparkles className="w-4 h-4" /></>
            )}
          </button>
        </div>
      </div>
