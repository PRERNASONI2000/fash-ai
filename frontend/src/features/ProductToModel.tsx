//feature/ProductToModel.tsx
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { runGeneration } from "../lib/fashnService";
import { Upload, Sparkles, Image as ImageIcon, User, Loader2, X, Download, Plus } from "lucide-react";
...
// Credit Calculation Logic (Based on Fashn Docs)
const calculateCredits = (mode: string, res: string, hasFaceRef: boolean, numImages: number) => {
  ...
  if (hasFaceRef) baseCost += 3; // +3 for face reference
  return baseCost * numImages;
};

export function ProductToModel() {
  const navigate = useNavigate();
  // Input States
  const [productImages, setProductImages] = useState<string[]>([]);
  const [prompt, setPrompt] = useState<string>("");
  const [faceReference, setFaceReference] = useState<string>("");
  ...
  const estimatedCredits = calculateCredits(generationMode, resolution, !!faceReference, numImages);
  ...
  const handleGenerate = async () => {
    if (productImages.length === 0) { // ✅ Changed condition
    ...
    try {
      const inputs: Record<string, any> = {
        product_image: productImages[0], // ✅ API takes the first selected image
        prompt: prompt || undefined,
        aspect_ratio: aspectRatio,
        resolution: resolution,
        generation_mode: generationMode,
        num_images: numImages,
        face_reference: faceReference || undefined,
        output_format: "png" // default
      };

      const data = await runGeneration("product-to-model", inputs);
      
      // Check if output is array (multiple images) or single string
   ...
   // Export & Download Logic using Canvas (To bypass CORS and apply format/quality)
  const handleExportDownload = () => {
    const img = new Image();
    img.crossOrigin = "Anonymous"; // Required to bypass canvas taint
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-[#f2eee6] to-[#faf9f7] dark:from-[#121212] dark:to-[#1a1a1a] transition-colors duration-300 flex flex-col">
      ...
      <div className="pt-8 pb-4 text-center border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-black/20 backdrop-blur-sm">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Transform product images into realistic AI models</p>
      </div>
        ...
        <div className="lg:w-1/2 flex flex-col gap-5 overflow-y-auto pb-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800">
            <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 flex items-center gap-2 mb-3">
              <ImageIcon className="w-4 h-4 text-[#b5652a]" /> Product Image (Required)
            </label>
              ...
              <input 
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    if (val) { setProductImages(prev => [...prev, val]); (e.target as HTMLInputElement).value = ""; }
                  }
                }}
                className="mt-3 w-full text-sm border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 rounded-md p-2 focus:ring-1 focus:ring-[#b5652a] outline-none"
              />
            )}
          </div>

          {/* Prompt & Face Reference moved below image selection */}
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800 space-y-3">
            <div>
              <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 mb-1 block">Prompt (Optional)</label>
              <input 
                type="text"
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="e.g. Blonde hair, studio photoshoot, elegant pose"
                className="w-full border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 rounded-md p-2 text-sm focus:ring-1 focus:ring-[#b5652a] outline-none"
              />
            </div>
            <div>
              <label className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 flex items-center gap-2 mb-1">
                <User className="w-4 h-4 text-[#b5652a]" /> Face Reference (Optional)
              </label>
              <input 
                type="text"
                value={faceReference}
                onChange={(e) => setFaceReference(e.target.value)}
                placeholder="Paste face URL for consistent identity (+3 credits)"
                className="w-full border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 rounded-md p-2 text-sm focus:ring-1 focus:ring-[#b5652a] outline-none"
              />
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800 space-y-4">
            ...
            <div>
              ...
              <SettingToggle options={[{label:"Fast",value:"fast"},{label:"Balanced",value:"balanced"},{label:"Quality",value:"quality"}]} current={generationMode} setter={setGenerationMode} />
            </div>
            <div>
              <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-2 block">Number of Images</label>
              <SettingToggle options={[{label:"1 Image",value:1},{label:"2 Images",value:2},{label:"3 Images",value:3}]} current={numImages} setter={setNumImages} />
            </div>
          </div>
        </div>
        ...
        <div className="lg:w-1/2 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800 flex flex-col min-h-[500px] relative">
          {loading && (
            <div className="absolute inset-0 bg-white/80 dark:bg-zinc-900/90 flex flex-col items-center justify-center z-10 rounded-xl backdrop-blur-sm">
              ...
              <p className="font-medium text-zinc-800 dark:text-white">Generating your models...</p>
            </div>
          )}
          <div className="p-4 border-b dark:border-zinc-800 flex items-center justify-between">
            ...
            {results.length > 0 && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">{results.length} Generated</span>}
          </div>
          <div className="flex-1 p-4 overflow-y-auto">
           {results.map((url, idx) => (
              <div key={idx} className="relative bg-zinc-50 dark:bg-zinc-800 rounded-lg overflow-hidden mb-4 h-64 flex items-center justify-center border border-zinc-100 dark:border-zinc-700 group">
      ...
      <div className="sticky bottom-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-t border-zinc-200 dark:border-zinc-800 p-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button onClick={handleGenerate} disabled={loading || productImages.length === 0} className="px-8 py-3 rounded-xl bg-gradient-to-r from-[#b5652a] to-[#d97a40] text-white font-semibold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 hover:scale-105 active:scale-95">
            {loading ? <>Processing...</> : <>Run Generation <Sparkles className="w-4 h-4" /></>}
          </button>
        </div>
      </div>
